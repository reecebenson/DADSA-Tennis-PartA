...
class Menu():
    ...
    def get_menu_name(self, index):
        try:
            menu_name = [ (v) for k,v in enumerate(self._menu[self.get_current_menu_index()]) if(k == index) ][0]
            return menu_name
        except IndexError:
            return None
    ...
    def get_input(self):
        ...
        try:
            ...
            # Attempt index
            new_menu = self.get_menu_name(resp)
            if(new_menu != None):
                print("menu exists")
                self.set_current_menu_index(int(resp))
                self.display()
            # Check if we're selecting our go back option
            elif((resp + 1) == len(self.get_menu(self.get_current_menu_index())) and self.get_current_menu_index() != 0):
                print("go back")
            else:
                print("non existant menu:", self.get_menu(self.get_current_menu_index()))
