        ...
        for seasonId in self.get_seasons():
            ...
            for gender in players:
                ...
                for r in range(2, season.settings()['round_count'] + 1):
                    ...
                    winners = season.round(gender, "round_"+str(r - 1)).winners()

                    rand_players = random.sample(winners, len(winners))
                    for winner in range(len(rand_players) // 2):
                        print("debug", winners[winner].name(), r, round_name)

                        # Grab our versus players
                        p_one = rand_players[winner * 2]
                        p_two = rand_players[(winner * 2) + 1]
                    ...
                    # Append our round to our season
                    print(season.add_round(gender, _round).winners())
