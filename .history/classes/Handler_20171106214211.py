...
class Handler():
    ...
    def load_prize_money(self):
        with open('./data/rankingPoints.json') as tData:
            ...
            [ self.prize_money[rank][rank,pts] for pts in data for rank in data[pts] ]
            #self.prize_money = [ [i,0] for i in range(self.player_count) for pts in data for rank in data[pts] ]

            # Set the prize money to the actual rank and points received
            #self.prize_money = [ [rank,int(pts)] for pts in data for rank in data[pts] ]
