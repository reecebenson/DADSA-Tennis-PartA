        ...
        for seasonId in self.get_seasons():
            ...
            for gender in players:
                ...
                for r in range(2, season.settings()['round_count'] + 1):
                    ...
                    print("prev: {0} has {1} matches".format(prev_round_name, _prev_round.match_count()))

                    # Items in Round
                    print("{0} has {1} matches\n".format(_round.name(), _round.match_count()))
                    _round.add_match(Match.Match(_round, players["male"][0], players["male"][1], 3, 2))
